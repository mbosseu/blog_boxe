...
- Le secret GitHub `GROQ_API_KEY` active le moteur cloud. Sans ce secret, le workflow se termine proprement sans rechercher ni publier. Les variables facultatives `GROQ_RESEARCH_MODEL` et `GROQ_WRITING_MODEL` permettent de changer les modèles ; leurs valeurs par défaut sont respectivement `openai/gpt-oss-20b` et `openai/gpt-oss-120b`.
